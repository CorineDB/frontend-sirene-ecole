  ...
  <DashboardLayout>
    <div class="p-6 space-y-6">
        ...
        >
          ...
          <div class="space-y-3 mb-4">
            ...
            <div class="flex items-start gap-2">
              ...
              <div>
                <p class="text-sm font-semibold text-gray-900">{{ ordre.panne.ecole.nom }}</p>
                <p class="text-xs text-gray-600">{{ ordre.panne.site.adresse }}, {{ ordre.panne.site.ville.nom_complet }}</p>
              </div>
            </div>
          ...
          <div class="flex items-center justify-between pt-4 border-t border-gray-100">
            <div class="flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>              {{ ordre.panne.description }}</span>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <router-link
          v-for="order in displayedOrdres"
          :key="order.id"
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div>
              <div class="flex items-center gap-2 mb-2">
                <h3 class="text-lg font-bold text-gray-900">{{ order.numero_ordre }}</h3>
                <span :class="`inline-block px-2 py-1 rounded-full text-xs font-semibold ${priorityColors[order.priorite]}`">
                  {{ priorityLabels[order.priorite] }}
                </span>
              </div>
              <div class="flex items-center gap-2">
                <span :class="`inline-block px-3 py-1 rounded-full text-xs font-semibold ${statusColors[order.statut]}`">
                  {{ statusLabels[order.statut] }}
                </span>
                <span :class="`inline-block px-3 py-1 rounded-full text-xs font-semibold ${typeColors[order.panne.type]}`">
                  {{ typeLabels[order.panne.type] }}
                </span>
              </div>
            </div>
          ...
          <div class="space-y-3">
            <div class="flex items-start gap-2">
              <MapPin :size="16" class="text-gray-400 mt-1 flex-shrink-0" />
              <div>
                <p class="text-sm font-semibold text-gray-900">{{ order.panne.ecole.nom }}</p>
                <p class="text-xs text-gray-600">{{ order.panne.site.adresse }}, {{ order.panne.site.ville.nom_complet }}</p>
              </div>
            </div>
...
import {
  ...
  AlertCircle,
  Clock,
  CheckCircle,ClipboardList, Plus
...
const stats = computed(() => [
  {
    label: 'Total',
    value: ordresMission.value.length,
    color: 'from-blue-500 to-blue-600',
    icon: FileText,
  },
  {
    label: 'En attente',
    value: ordresMission.value.filter(o => o.status === 'pending').length,
    color: 'from-yellow-500 to-yellow-600',
    icon: Clock,
  },
  {
    label: 'En cours',
    value: ordresMission.value.filter(o => o.status === 'in_progress').length,
    color: 'from-cyan-500 to-cyan-600',
    icon: ClipboardList,
  },
  {
    label: 'Terminées',
    value: ordresMission.value.filter(o => o.status === 'completed').length,
...
const displayedOrdres = computed(() => {
  return ordresMission.value.filter(order => {
