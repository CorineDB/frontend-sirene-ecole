    ...
    <div class="bg-white rounded-xl max-w-lg w-full" @click.stop>
      <div class="p-6">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xl font-bold text-gray-900">Ajouter un Technicien à la Mission</h3>
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="flex gap-4 pt-4">
            ...
            >
              {{ submitting ? 'Assignation...' : 'Ajouter le Technicien' }}
            </button>
            <button
              type="button"
              @click="$emit('close')"
...
const { assignerTechnicien } = useOrdresMission();
...
const initialize = async () => {
  if (!props.missionId) {
    notificationStore.error("ID de mission manquant pour assigner un technicien.");
    emit('close');
    return;
  }
  loading.value = true;
...
const handleSubmit = async () => {
  if (!props.missionId || !form.value.technicien_id) {
  ...
  try {
    await assignerTechnicien(props.missionId, form.value.technicien_id, form.value.role || undefined);
    notificationStore.success("Technicien assigné à la mission avec succès !");
    emit('success');
    emit('close');
  } catch (err) {
    notificationStore.error("Erreur lors de l'assignation du technicien à la mission.");
    console.error(err);
  } finally {
